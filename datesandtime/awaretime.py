...
local_time = _datetime.datetime.now()
utc_time = _datetime.datetime.utcnow()
...
gap_time = _datetime.datetime(2015, 10, 25, 1, 30, 0, 0)
print(gap_time)
...
t = s + (60 * 60)
gb = pytz.timezone('GB')
dt1 = pytz.utc.localize(_datetime.datetime.utcfromtimestamp(s)).astimezone(gb)
dt2 = pytz.utc.localize(_datetime.datetime.utcfromtimestamp(t)).astimezone(gb)
